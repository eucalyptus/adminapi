...
class AutoCreds(Eucarc):
    ...
    def _legacy_create_credentials(self, clc, admin_cred_dir, account, user, zipfile='creds.zip'):
        ...
        if output['status'] == 0:
            self.log.debug("Found creds file, skipping euca_conf --get-credentials.")
        else:
    ...
    def get_active_cert_for_creds(self, credzippath=None, account=None, user=None, update=True,
        ...
        self.log.debug('Current EC2_CERT path for {0}: {1}'.format(clc_eucarc, certpath_in_eucarc))
        if certpath_in_eucarc and self.get_active_id_for_cert(certpath_in_eucarc):
            self.log.debug("Cert/pk already exist and is active in '" +
            ...
            self.log.debug('Attempting to find an active cert for this account on the CLC...')
            certpaths = (self.find_active_cert_and_key_in_dir(dir=admin_cred_dir) or
                         self.find_active_cert_and_key_in_dir())
            self.log.debug('Found Active cert and key paths')
            if not certpaths:
                # No existing and active certs found, create new ones...
                self.log.debug('Could not find any existing active certs on clc, '
            ...
            self.log.debug('Using certpath:{0} and keypath:{1} on clc'
            ...
            self.log.debug("Setting cert/pk in '" + admin_cred_dir + "/eucarc'")
            ...
            self.log.debug('updating zip file with new cert, key and eucarc: {0}'
                       .format(credzippath))
    ...
    def create_new_user_certs(self, admin_cred_dir, account, user, force_cert_create=False,
        ...
        if len(admin_certs) > 1:
            if force_cert_create:
                self.log.debug("Found more than one certs, deleting last cert")
        ...
        self.log.debug("Creating a new signing certificate for user '{0}' in account '{1}'."
                   .format(user, account))
        self.log.debug('New cert name:{0}, keyname:{1}'.format(os.path.basename(newcertpath),
    ...
    def download_certs_from_clc(self, admin_cred_dir=None, update_eucarc=True, machine=None):
        ...
        self.log.debug("Downloading certs from " + self.clc.hostname + ", path:" +
                   admin_cred_dir + "/")
        clc_eucarc = os.path.join(admin_cred_dir, 'eucarc')
        ...
        if update_eucarc:
            self.log.debug("Setting cert/pk in '{0}".format(local_eucarc))
    ...
    def send_creds_to_machine(self, admin_cred_dir, machine, filename='creds.zip'):
        ...
        self.log.debug("Sending credentials to " + machine.hostname)
        localmd5 = None
        remotemd5 = None
        try:
        ...
        if not remotemd5 or (remotemd5 != localmd5):
            ...
            self.log.debug("Machine " + machine.hostname + " already has credentials in place not "
                                                       " sending")
